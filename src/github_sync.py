...
async def push_file_to_github(repo, path, content_str, token, branch="main", message=None):
    """
    Заливает файл в GitHub через API. Content_str - обычный текст, внутри закодируем в base64.
    Возвращает raw_url или None
    """
    if not token:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.put(url, headers=headers, json=payload) as resp:
            ...
            if resp.status in (200, 201):
                logger.info(f"✅ GitHub push OK: {path} -> {resp.status}")
